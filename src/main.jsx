    ...
    <section className="home-section">
      ...
      <div className="commerce-categories">
        {categories.slice(0,8).map(c=><button key={c.id||c.name} className="commerce-category glass interactive-card" onClick={()=>{setQuery(c.name);setPage('catalog');}}>
          <div><img src={c.image_url||categoryIcons[c.name]||'/tool-drill.svg'}/></div><span>{c.name}</span><small>Перейти →</small>
        </button>)}
      </div>
    </section>
...
function ProductPage({selected,products,open,add,favs,toggleFav,compare,toggleCompare,setPage}){
  ...
  React.useEffect(()=>{setActiveImage(0);setImageZoomOpen(false)},[selected.id]);
  ...
  return <section className="product-page-v7 product-page-v9">
    ...
    <div className="product-main-v7">
      <div className="product-gallery-v7 glass">
        <button type="button" className="gallery-main interactive-card product-image-zoom" onClick={()=>setImageZoomOpen(true)} aria-label="Увеличить изображение"><img src={gallery[activeImage]||productImage(selected)}/><span className="zoom-hint"><Search/> Увеличить</span></button>
    ...
    {imageZoomOpen&&<div className="product-lightbox" onClick={()=>setImageZoomOpen(false)}><button className="product-lightbox-close" onClick={()=>setImageZoomOpen(false)}><X/></button><div className="product-lightbox-stage" onClick={e=>e.stopPropagation()}><img src={gallery[activeImage]||productImage(selected)} alt={selected.title}/></div></div>}
...
function Store({products,categories,setAdmin}){
  ...
  const add=p=>{
    ...
    setCart(x=>{
      ...
      if(f){
        if(f.qty>=stock){
          alert(`Можно добавить максимум ${stock} шт.`);
          return x;
        }
        return x.map(i=>i.id===p.id?{...i,qty:i.qty+1}:i);
      }
      return [...x,{...p,qty:1}];
    });
    setCartOpen(true);
  ...
  return <div className="app commerce-app">
    ...
    <aside className={'cart-drawer commerce-cart glass '+(cartOpen?'open':'')}>
      ...
      <div className="cart-list">{cart.length===0?<div className="empty"><ShoppingCart size={38}/><span>Корзина пуста</span></div>:cart.map(i=><div className="cart-item" key={i.id}><img src={productImage(i)}/><span><b>{i.title}</b><small>{rub(i.price)}</small>{Number(i.stock)<=0&&<em className="cart-out-stock">Нет в наличии</em>}</span><div className="qty"><button onClick={()=>setCart(x=>x.map(v=>v.id===i.id?{...v,qty:Math.max(1,v.qty-1)}:v))}><Minus/></button>{i.qty}<button disabled={i.qty>=Number(i.stock||0)} onClick={()=>setCart(x=>x.map(v=>v.id===i.id&&v.qty<Number(v.stock||0)?{...v,qty:v.qty+1}:v))}><Plus/></button></div><button className="cart-remove" title="Удалить" onClick={()=>setCart(x=>x.filter(v=>v.id!==i.id))}><Trash2/></button></div>)}</div>
      <div className="drawer-total"><span>Итого</span><b>{rub(total)}</b></div>
      <button className="primary cart-checkout" disabled={!cart.length||cart.some(i=>Number(i.stock)<=0||i.qty>Number(i.stock||0))} onClick={()=>{setCartOpen(false);nav("checkout")}}>Перейти к оформлению</button>
    </aside>
...
function AdminLogin({onDone,onCancel}){
  ...
  return <div className="admin-login"><DrillCursor page="admin-login"/><form className="glass" onSubmit={login}><img src="/zona-logo.png"/><span className="eyebrow">ADMIN</span><h1>Вход в управление</h1><input type="email" placeholder="Email администратора" value={email} onChange={e=>setEmail(e.target.value)}/><input type="password" placeholder="Пароль" value={password} onChange={e=>setPassword(e.target.value)}/>{error&&<p className="error">{error}</p>}<button className="primary">Войти</button><button type="button" className="ghost" onClick={onCancel}>Вернуться в магазин</button></form></div>
}
...
function Admin({onExit,onBack}){
 const [tab,setTab]=React.useState('products'),[products,setProducts]=React.useState([]),[categories,setCategories]=React.useState([]),[adminCategories,setAdminCategories]=React.useState([]),[categoryEdit,setCategoryEdit]=React.useState(null),[orders,setOrders]=React.useState([]),[loading,setLoading]=React.useState(true),[ordersLoading,setOrdersLoading]=React.useState(false),[edit,setEdit]=React.useState(null),[form,setForm]=React.useState(false),[expandedOrder,setExpandedOrder]=React.useState(null),[productSearch,setProductSearch]=React.useState(''),[categorySearch,setCategorySearch]=React.useState('');
 ...
 React.useEffect(()=>{load()},[]);React.useEffect(()=>{if(tab==='orders')loadOrders();if(tab==='categories')getAdminCategories().then(setAdminCategories).catch(e=>alert(e.message))},[tab]);
 ...
 const visibleAdminProducts=products.filter(p=>`${p.title||''} ${p.brand||''} ${p.category||''} ${p.spec||''} ${p.id||''}`.toLowerCase().includes(productSearch.trim().toLowerCase()));
 const visibleAdminCategories=adminCategories.filter(c=>`${c.name||''} ${c.slug||''}`.toLowerCase().includes(categorySearch.trim().toLowerCase()));
 return <div className="admin-shell"><DrillCursor page={"admin-"+tab}/><aside className="admin-side glass"><img src="/zona-logo.png"/><span className="admin-badge">ПАНЕЛЬ УПРАВЛЕНИЯ</span><nav><button className={tab==='dashboard'?'active':''} onClick={()=>setTab('dashboard')}><LayoutDashboard/>Обзор</button><button className={tab==='products'?'active':''} onClick={()=>setTab('products')}><Package/>Товары</button><button className={tab==='categories'?'active':''} onClick={()=>setTab('categories')}><Grid2X2/>Категории</button><button className={tab==='orders'?'active':''} onClick={()=>setTab('orders')}><ShoppingBag/>Заказы{newOrders>0&&<i className="admin-order-count">{newOrders}</i>}</button></nav><div className="side-bottom"><button className="admin-back-site" onClick={onBack}><StoreIcon/><span>На сайт</span></button><span className="online">● Node API + PostgreSQL</span><button className="admin-logout" onClick={logout}><LogOut/><span>Выйти</span></button></div></aside><main className="admin-main"><header><div><span className="eyebrow">ZONA CONTROL</span><h1>{tab==='products'?'Товары':tab==='categories'?'Категории':tab==='orders'?'Заказы':'Обзор магазина'}</h1></div>{tab==='products'&&<button className="primary" onClick={()=>{setEdit(null);setForm(true)}}>+ Добавить товар</button>}</header>{tab==='products'&&<div className="admin-search glass"><Search/><input value={productSearch} onChange={e=>setProductSearch(e.target.value)} placeholder="Поиск товара по названию, бренду, категории или ID"/>{productSearch&&<button onClick={()=>setProductSearch('')}><X/></button>}<span>{visibleAdminProducts.length} из {products.length}</span></div>}{tab==='categories'&&<div className="admin-search glass"><Search/><input value={categorySearch} onChange={e=>setCategorySearch(e.target.value)} placeholder="Поиск категории"/>{categorySearch&&<button onClick={()=>setCategorySearch('')}><X/></button>}<span>{visibleAdminCategories.length} из {adminCategories.length}</span></div>}{tab==='dashboard'&&<div className="stats"><div className="glass"><small>Товаров</small><strong>{products.length}</strong></div><div className="glass"><small>В наличии</small><strong>{products.reduce((a,b)=>a+(b.stock||0),0)}</strong></div><div className="glass"><small>Новых заказов</small><strong>{newOrders}</strong></div></div>}{tab==='products'&&<div className="admin-table glass">{loading?<div className="loading">Загрузка...</div>:visibleAdminProducts.map(p=><div className="admin-row" key={p.id}><img src={p.image_url||'/tool-drill.svg'}/><div className="admin-name"><b>{p.title}</b><small>{p.category} · {p.spec}</small><em className={'sync-badge '+(p.sync_source==='digit'?'digit':'manual')}>{p.sync_source==='digit'?'DIGIT':'САЙТ'}</em></div><strong>{rub(p.price)}</strong><span>{p.stock} шт.</span><span className={p.is_active?'status on':'status'}>{p.is_active?<><Eye/>Активен</>:<><EyeOff/>Скрыт</>}</span><button onClick={()=>{setEdit(p);setForm(true)}}><Pencil/></button><button className="danger" onClick={()=>del(p)}><Trash2/></button></div>)}</div>}{tab==='categories'&&<div className="admin-categories-grid">{visibleAdminCategories.map(c=><article className={'admin-category-card glass '+(!c.is_active?'inactive':'')} key={c.id}><div className="admin-category-image"><img src={c.image_url||categoryIcons[c.name]||'/tool-drill.svg'}/></div><div className="admin-category-info"><b>{c.name}</b><small>DIGIT УЧЁТ · порядок {c.sort_order}</small><span className={c.is_active?'cat-status on':'cat-status'}>{c.is_active?'Показывается':'Скрыта'}</span></div><button className="icon-btn" onClick={()=>setCategoryEdit(c)}><Pencil/></button></article>)}</div>}{tab==='orders'&&<div className="orders-admin">{ordersLoading?<div className="loading glass">Загрузка заказов...</div>:orders.length===0?<div className="empty-admin glass"><ShoppingBag/><h2>Заказов пока нет</h2></div>:orders.map(o=><article className="admin-order-card glass" key={o.id}><div className="admin-order-head" onClick={()=>setExpandedOrder(expandedOrder===o.id?null:o.id)}><div><span className="order-number">{o.order_number}</span><small>{new Date(o.created_at).toLocaleString('ru-RU')}</small></div><div><b>{o.customer_name}</b><a href={`tel:${o.customer_phone}`}>{o.customer_phone}</a></div><div><span>{o.delivery_label}</span><strong>{rub(o.total)}</strong></div><select value={o.status} onClick={e=>e.stopPropagation()} onChange={e=>changeStatus(o,e.target.value)}>{Object.entries(ORDER_STATUS).map(([v,l])=><option value={v} key={v}>{l}</option>)}</select><ChevronDown className={expandedOrder===o.id?'rotated':''}/></div>{expandedOrder===o.id&&<div className="admin-order-body"><div className="admin-order-items">{o.items.map(i=><div key={i.id}><span>{i.title}<small>{i.quantity} × {rub(i.price)}</small></span><b>{rub(i.line_total)}</b></div>)}</div><div className="admin-order-details"><p><b>Получение:</b> {o.delivery_label}</p>{o.delivery_address&&<p><b>Адрес / ПВЗ:</b> {o.delivery_address}</p>}<p><b>Оплата:</b> {o.payment_type==='cash'?'Наличными':'Переводом'}</p>{o.customer_email&&<p><b>Email:</b> {o.customer_email}</p>}{o.comment&&<p><b>Комментарий:</b> {o.comment}</p>}<a className="admin-wa-button" href={`https://wa.me/${String(o.customer_phone||'').replace(/\D/g,'')}`} target="_blank" rel="noreferrer"><MessageCircle/>Написать покупателю</a></div></div>}</article>)}</div>}</main>{form&&<ProductForm item={edit} categories={categories} onSave={save} onCancel={()=>{setForm(false);setEdit(null)}}/>}{categoryEdit&&<CategoryForm item={categoryEdit} onCancel={()=>setCategoryEdit(null)} onSave={async data=>{try{await editCategory(categoryEdit.id,data);setCategoryEdit(null);setAdminCategories(await getAdminCategories());setCategories(await getCategories())}catch(e){alert(e.message)}}}/>}</div>
}
...
function App(){
  ...
  if(admin&&logged)return <Admin onBack={()=>setAdmin(false)} onExit={()=>{setAdmin(false);setLogged(false)}}/>;
